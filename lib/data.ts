...
import { seedProfile, seedStartups, seedTech, seedProjects, seedPosts } from "./seed";
import type { Profile, Startup, Tech, Project, Post, Message } from "./types";
...
export async function getPublicContent() {
  const [profile, startups, tech, projects] = await Promise.all([
    getProfile(),
    getStartups(),
    getTech(),
    getProjects(),
  ]);
  return { profile, startups, tech, projects };
}
